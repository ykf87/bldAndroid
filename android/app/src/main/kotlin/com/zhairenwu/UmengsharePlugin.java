...
/** UmengsharePlugin */
public class UmengsharePlugin implements FlutterPlugin, ActivityAware,MethodCallHandler,ActivityResultListener,RequestPermissionsResultListener {
  private  static MethodChannel channel;
  public  Activity context;

  public UmengsharePlugin(){
    init();
  }
...
//      PlatformConfig.setQQZone("100424468", "c7394704798a158208a74ab60104f0ba");
  ...
  private void onAttachedToEngine(Context applicationContext, BinaryMessenger messenger) {
    channel = new MethodChannel(messenger, "ugle.cn/UMengShare");
  ...
  public void onMethodCall(MethodCall call, Result result) {
    if (call.method.equals("shareText")) {
      ...
      int platform=call.argument("platform");
      boolean flag=UMShareAPI.get(context).isInstall((Activity) context, getPlatForm(platform));
      result.success(flag);
    }
    else {
  ...
  private void shareText(SHARE_MEDIA platform, String text, final Result result) {
    new ShareAction( context).setPlatform(platform)
            .withText(text)
            .setCallback(new UmengshareActionListener(context,result)).share();
  }
  ...
  private void shareMiniApp( String username,String title, String desc, String thumb, String url,String path ,final Result result){
    ...
    umMin.setThumb(new UMImage(context,thumb));
  ...
  public boolean onActivityResult(int i, int i1, Intent intent) {
    UMShareAPI.get(context).onActivityResult(i, i1, intent);
    return false;
  }
