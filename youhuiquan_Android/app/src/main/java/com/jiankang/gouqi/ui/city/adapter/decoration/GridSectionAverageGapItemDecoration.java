package com.jiankang.gouqi.ui.city.adapter.decoration;
    ...
    private void markSections() {
        if (mAdapter != null) {
            ...
            if (mAdapter.hasHeaderLayout()) {
                size = size - mAdapter.getHeaderLayoutCount();
            }
            if (mAdapter.hasFooterLayout()) {
                size = size - mAdapter.getFooterLayoutCount();
            }
