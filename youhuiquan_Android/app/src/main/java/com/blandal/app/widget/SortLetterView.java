...
public class SortLetterView extends View {
    ...
    // 字母默认字体大小
    float mLetterSize = SortLetterView.dp2px(12);
    //  被选中的左边大字母大小
    float leftBigTextSize = dp2px(26);
    //右边间距
    float paddingRight = dp2px(12);
    //左边选中放大背景图片宽度
    float iconWidth = dp2px(58);
    //左边选中放大背景图片高度
    float iconHeight = dp2px(49);
    //左边放大图标和右边字母的距离
    float leftIconPadding = dp2px(30);
    //字母上下的距离
    float letterPadding = dp2px(5);
    //左边背景图片id
    int backgroundIconId;
    //左边放大字母偏移x
    float offSetX = dp2px(4);
    //模式选择
    ...
    private Paint mLetterBgPaint;//背景
    ...
    private boolean isActionUP;//手指抬起时取消左边大字母
    //字母触摸选中宽度范围
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (backgroundIconId == 0) {
            mBitmap = getBitmap(getContext(), R.drawable.conner);
        } else {
            mBitmap = getBitmap(getContext(), backgroundIconId);
        }
        newBmp = Bitmap.createScaledBitmap(mBitmap, (int) iconWidth, (int) iconHeight, true);

        mLetterBgPaint = new Paint();
        mLetterBgPaint.setColor(0xFFdfe6ec);
        mLetterBgPaint.setStyle(Paint.Style.STROKE);
        mLetterBgPaint.setStrokeWidth(1);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mHeight = i;
        mWidth = w;
        dispatchWidth = mWidth - getPaddingRight() - mLetterSize - paddingRight - dp2px(14);
    }

    @Override
    protected void onDraw(Canvas canvas) {
        RectF rect = new RectF(mWidth - mLetterSize - paddingRight, getPaddingTop() + dp2px(8), mWidth, mHeight + iconHeight);
        // 边框圆角弧度
        canvas.drawRoundRect(rect, 20f, 20f, mLetterBgPaint);
